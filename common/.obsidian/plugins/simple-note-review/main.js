...
var NoteSetService = class {
  ...
  updateAllNotesetErrors() {
    return __async(this, null, function* () {
      yield Promise.all(this._plugin.settings.noteSets.map((noteset) => this.validateRules(noteset)));
    });
  }
  validateRules(noteSet) {
    return __async(this, null, function* () {
      this._plugin.settings.noteSets.find((x) => x.id === noteSet.id).validationError = yield this.notesetRuleError(noteSet);
      yield this._plugin.saveSettings();
    });
  }
  notesetRuleError(noteset) {
    return __async(this, null, function* () {
      var _a, _b;
...
var SelectNoteSetModal = class extends import_obsidian.SuggestModal {
  ...
  onChooseSuggestion(noteSet, evt) {
    return __async(this, null, function* () {
      var _a;
      try {
        yield this._plugin.reviewService.startReview(noteSet);
...
var SimpleNoteReviewPluginSettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    this._plugin.settings && this._plugin.settings.noteSets && this._plugin.settings.noteSets.forEach((noteSet, index) => {
      ...
      if (noteSet == null ? void 0 : noteSet.validationError) {
        setting.addExtraButton((cb) => {
          cb.setIcon("alert-triangle").setTooltip(noteSet == null ? void 0 : noteSet.validationError);
        });
      }
      setting.addExtraButton((cb) => {
      ...
      setting.addExtraButton((cb) => {
        cb.setIcon("rotate-cw").setTooltip("Reset review queue and update stats for this note set").onClick(() => __async(this, null, function* () {
          yield this._plugin.noteSetService.validateRules(noteSet);
          yield this._plugin.reviewService.resetNotesetQueue(noteSet);
...
var _SimpleNoteReviewSidebarView = class extends import_obsidian6.ItemView {
  ...
  createCurrentFileActionsEl(parentEl) {
    ...
    actionsEl.addExtraButton((cb) => {
      cb.setIcon("ban").setTooltip("ignore this note in all reviews").onClick(() => {
        this._plugin.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.ignore);
      });
    });
    actionsEl.addExtraButton((cb) => {
      cb.setIcon("signal-low").setTooltip("set review frequency to low").onClick(() => {
        this._plugin.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.low);
      });
    });
    actionsEl.addExtraButton((cb) => {
      cb.setIcon("signal-medium").setTooltip("set review frequency to normal").onClick(() => {
        this._plugin.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.normal);
      });
    });
    actionsEl.addExtraButton((cb) => {
      cb.setIcon("signal").setTooltip("set review frequency to high").onClick(() => {
        this._plugin.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.high);
      });
    });
    actionsEl.addExtraButton((cb) => {
      cb.setIcon("skip-forward").setTooltip("skip note for current review").onClick(() => {
        this._plugin.reviewService.skipNote(this.app.workspace.getActiveFile(), this._plugin.settings.currentNoteSet);
      });
    });
    actionsEl.addExtraButton((cb) => {
      cb.setIcon("file-check").setTooltip("mark current note as reviewed & go to the next file").onClick(() => {
        this._plugin.reviewService.reviewNote(this.app.workspace.getActiveFile(), this._plugin.settings.currentNoteSet);
      });
    });
    return actionsEl.settingEl;
  }
  createNotesetSection(noteSet) {
    ...
    section.addExtraButton((cb) => {
      cb.setIcon("dices").setTooltip("open random note from this note set").onClick(() => __async(this, null, function* () {
        return this.startReviewWithDelegate(noteSet, this._plugin.reviewService.openRandomNoteInQueue);
      }));
    });
    section.addExtraButton((cb) => {
      cb.setIcon("rotate-cw").setTooltip("reset review queue for this note set").onClick(() => __async(this, null, function* () {
        return yield this._plugin.reviewService.resetNotesetQueue(noteSet);
      }));
    });
    section.addExtraButton((cb) => {
      cb.setIcon("play").setTooltip("review this note set").onClick(() => __async(this, null, function* () {
        return this.startReviewWithDelegate(noteSet, this._plugin.reviewService.startReview);
      }));
    });
  ...
  startReviewWithDelegate(noteSet, delegate) {
    return __async(this, null, function* () {
      var _a;
      try {
        yield delegate.bind(this._plugin.reviewService)(noteSet);
      } catch (error) {
        if (error instanceof NoteSetEmptyError) {
          this._plugin.showNotice(`note set ${(_a = noteSet.displayName) != null ? _a : noteSet.name} is empty.`);
        }
...
var ReviewService = class {
  ...
  resetNotesetQueue(noteset) {
    return __async(this, null, function* () {
      yield this.createNotesetQueue(noteset);
    });
  }
  reviewNote(note, noteSet) {
    return __async(this, null, function* () {
...
var SimpleNoteReviewPlugin = class extends import_obsidian9.Plugin {
  ...
  loadSettings() {
    return __async(this, null, function* () {
      ...
      yield this.noteSetService.updateAllNotesetErrors();
    });
  }
  saveSettings() {
    return __async(this, null, function* () {
  ...
  addCommands() {
    ...
    this.addCommand({
      ...
      callback: () => {
        this.reviewService.openRandomNoteInQueue(this.settings.currentNoteSet);
      }
    });
    this.addCommand({
      id: "reset-queue",
      ...
      callback: () => {
        this.reviewService.resetNotesetQueue(this.settings.currentNoteSet);
      }
    });
    this.addCommand({
      id: "open-modal",
    ...
    this.addCommand({
      ...
      callback: () => {
        this.reviewService.reviewNote(this.app.workspace.getActiveFile(), this.settings.currentNoteSet);
      }
    });
    this.addCommand({
      ...
      callback: () => {
        this.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.high);
      }
    });
    this.addCommand({
      ...
      callback: () => {
        this.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.normal);
      }
    });
    this.addCommand({
      ...
      callback: () => {
        this.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.low);
      }
    });
    this.addCommand({
      ...
      callback: () => {
        this.fileService.setReviewFrequency(this.app.workspace.getActiveFile(), ReviewFrequency.ignore);
      }
    });
    this.addCommand({
      id: "skip-note",
      ...
      callback: () => {
        this.reviewService.skipNote(this.app.workspace.getActiveFile(), this.settings.currentNoteSet);
      }
    });
  }
  activateView() {
